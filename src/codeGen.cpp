...
void generateCode() {
  ...
  for (int i = 0; i < emittedCode.size(); ++i) {
    ...
    if (emittedCode[i].op.first=="GOTO")
      unconditionalgoto = 0;
    ...
    } else if (emittedCode[i].stmtNum == -4) {
      ...
      if (emittedCode[i].op.first == "param") {
        if (emittedCode[i].id1.second != NULL) {
          if (emittedCode[i].id1.second != NULL) {
            if (emittedCode[i].id1.second->is_init == -5) {
              ...
              addLine("mflo $t9");
              addLine("li $s6, " +
                to_string(emittedCode[i].id1.second->offset));
          ...
          counter++;
                ...
                emittedCode[i].op.first == "realtoint" ||
                emittedCode[i].op.first == "inttoreal") {
        ...
        if (emittedCode[i].res.second->is_init == -5) {
          if (currFunction == "main") {
            ...
            addLine("mflo $t9");
            addLine("li $s6, " +
                to_string(emittedCode[i].res.second->offset));
            addLine("add $s6, $s6, $t9");
            addLine("sub $s7, $fp, $s6");
          } else {
            addLine("li $s6, "+to_string(emittedCode[i].res.second->size) );
